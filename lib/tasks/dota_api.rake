...
task :dota2 => :environment do
   mechanize = Mechanize.new
    ...
    def scrape_hd_img(link,thumbnail_url,api_type,link_url)
      sleep(1)
      mechanize = Mechanize.new
      page = mechanize.get(link)
      url = link
      ...
      item = ApiItem.find_or_create_by(image: img_src,api_type: doto_type,image_thumbnail: thumbnail_url,link: link_url)
      if item.save
    ...
    def scrape_category_img(url)
      ...
      wallpapers.each do |wallpaper|
        ...
        scrape_hd_img(hd_image_link,img_src,"dota2_category_images",img_link)
      end
      paginate_link(url)
    end
